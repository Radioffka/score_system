...
const PointsControl: React.FC<PointsControlProps> = ({ reasons, onPointsChange, onSetPoints, disabled = false }) => {
  ...
  const handleCustomAdd = () => {
    ...
    if (!isNaN(amount)) {
      const description = customReason.trim() || `Manuální přidání`;
      onPointsChange(amount, description);
      setCustomAmount('');
      setCustomReason('');
    }
  };
  ...
  const handleCustomSubtract = () => {
    const amount = parseInt(customAmount, 10);
    if (!isNaN(amount)) {
      const description = customReason.trim() || `Manuální odebrání`;
      onPointsChange(-amount, description);
      setCustomAmount('');
      setCustomReason('');
    }
  ...
  const fieldsetClassName = `transition-opacity ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`;

  return (
    <div className="bg-gray-800 rounded-xl shadow-lg p-6">
      <h3 className="text-xl font-bold mb-4">Změna bodů</h3>
      <div className="space-y-6">
        <fieldset disabled={disabled} className={fieldsetClassName}>
            {/* Quick Change */}
            <div>
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-md font-semibold transition-colors ${val > 0 ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} disabled:bg-gray-500`}
                >
                    {val > 0 ? '+':''}{val}
                </button>
                ))}
            ...
            <div>
                ...
                <div className="flex gap-2">
                    <select
                        ...
                        className="flex-grow bg-gray-700 border border-gray-600 rounded-md py-2 px-3 focus:ring-teal-500 focus:border-teal-500 disabled:bg-gray-600"
                    >
                ...
                <div className="space-y-2">
                    <div className="flex flex-wrap gap-2">
                     ...
                     <input
                        ...
                        placeholder="Důvod (volitelný)"
                        className="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 focus:ring-teal-500 focus:border-teal-500 disabled:bg-gray-600"
                    />
                </div>
            </div>
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={handleReset} className="bg-yellow-600 hover:bg-yellow-700 font-bold py-2 px-4 rounded-md transition-colors disabled:bg-gray-500">
                        Reset (0)
                    </button>
                </div>
            </div>
